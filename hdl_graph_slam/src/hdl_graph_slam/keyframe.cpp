...
const pcl::PointCloud<PointT>::ConstPtr& cloud ,const pcl::PointCloud<PointRGBT>::ConstPtr cloudRGB) : stamp(stamp), odom(odom),
                                                                                                        accum_distance(accum_distance),cloud(cloud), cloudRGB(cloudRGB), node(nullptr) {}
...
KeyFrameSnapshot::KeyFrameSnapshot(const Eigen::Isometry3d& pose, const pcl::PointCloud<PointT>::ConstPtr& cloud,const pcl::PointCloud<PointRGBT>::ConstPtr cloudRGB) : pose(pose), cloud(cloud),cloudRGB(cloudRGB) {}

KeyFrameSnapshot::KeyFrameSnapshot(const KeyFrame::Ptr& key) : pose(key->node->estimate()), cloud(key->cloud) ,cloudRGB(key->cloudRGB) {}
